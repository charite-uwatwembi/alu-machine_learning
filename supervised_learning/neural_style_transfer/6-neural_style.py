...
class NST:
    ...
    def style_cost(self, style_outputs):
        ...
        if type(style_outputs) is not list or len(style_outputs) != length:
            raise TypeError(f"style_outputs must be a list of length {length}")

        style_cost = 0
        for i, style_output in enumerate(style_outputs):
            style_cost += self.layer_style_cost(style_output,
    ...
    def content_cost(self, content_output):
        ...
        returns:
            the content cost
        """
        # Ensure content_output is a tensor and has the same shape as self.content_feature
        ...
        # Calculate the squared differences between content_output and self.content_feature
        squared_diff = tf.square(content_output - self.content_feature)

        # Compute the content cost as the mean of the squared differences over height, width, and channels
        content_cost = tf.reduce_mean(squared_diff)
